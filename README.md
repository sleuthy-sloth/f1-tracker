...
**Status:** Early development (v0.1.0) — Phase 2 of 7 in progress
...
- **Reusable Card component** — 4 variants (default, glass, outlined, elevated) with configurable padding and hover states
- **SVG favicon** with SectorOne branding

### Planned (Phases 3–7)

- **Race Archive** — session browser with GP cards, circuit outlines, and filters
- **Strategy Lab** — canvas-based track map, replay playback engine, live leaderboard, driver telemetry HUD, gap charts, tyre degradation heatmaps, weather radar, and race control feed
...
```
├── app/
│   ├── globals.css          # Theme tokens, glassmorphism, scrollbar styling
...
├── lib/
│   ├── api/
│   │   └── openf1.ts        # OpenF1 API client (21 endpoints, rate limited)
│   ├── auth/
│   │   ├── AuthContext.tsx  # Firebase Auth context provider
│   │   └── useAuth.ts       # useAuth() hook
│   ├── firebase.ts          # Firebase config and lazy initialization
│   └── types.ts             # 28+ TypeScript types for F1 data entities
├── tests/
│   ├── verify-task2-2.test.ts
│   └── verification/
...
| Test File | Coverage |
|-----------|----------|
| `tests/verify-task2-2.test.ts` | Build verification, font/theme configuration |
| `tests/verification/navigation_verification.test.ts` | SideNav and MobileNav component exports and structure |
| `tests/verification/card_verification.test.tsx` | Card component API surface, variants, and padding levels |
...
| **2** | SectorOne Branding & Design System | 🔄 In Progress | Dark theme, glassmorphism, SideNav, MobileNav, Card component, landing page |
| **3** | Race Archive | 🔜 Planned | Session browser, GP cards, circuit outlines, filters |
| **4** | Strategy Lab | 🔜 Planned | Track map, replay engine, telemetry HUD, gap charts, weather radar |
